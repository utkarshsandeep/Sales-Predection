...
library(cowplot)
train = fread("Train_Dataset.csv")
train
test = fread("Test_Dataset.csv") 
test
sample = fread("Sample_Dataset.csv") 
sample
dim(train)
dim(test)
names(train)
...
ridge_linear_reg_mod = train(x = train[, -c("Item_Identifier", "Item_Outlet_Sales")], method='glmnet', trControl= my_control, tuneGrid = Grid)
